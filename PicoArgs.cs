...
public class PicoArgs(IEnumerable<string> args, bool recogniseEquals = true)
{
		...
		for (var index = 0; index < argList.Count; ++index) {
			if (optionsSet.Contains(argList[index].Key)) {
				...
				if (argList[index].Value != null) {
					throw new PicoArgsException(PicoArgsErrorCode.UnexpectedValue, $"Unexpected value for \"{string.Join(", ", options!)}\"");
				}
	...
	public string GetParam(params ReadOnlySpan<string> options)
		=> GetParamOpt(options) ?? throw new PicoArgsException(PicoArgsErrorCode.MissingRequiredParameter, $"Expected value for \"{string.Join(", ", options!)}\"");
	...
	private string? GetParamInternal(ReadOnlySpan<string> options)
	{
		...
		if (index == argList.Count - 1) {
			throw new PicoArgsException(PicoArgsErrorCode.MissingValue, $"Expected value after \"{item.Key}\"");
		}

		// grab and check the next parameter
		var secondItem = argList[index + 1];
		if (secondItem.Value != null) {
			throw new PicoArgsException(PicoArgsErrorCode.AmbiguousValue, $"Cannot identify value for param \"{item.Key}\", followed by \"{secondItem.Key}\" and \"{secondItem.Value}\"");
		}
	...
	/// </summary>
	public string GetCommand() => GetCommandOpt() ?? throw new PicoArgsException(PicoArgsErrorCode.MissingCommand, "Expected command");
	...
	public string? GetCommandOpt()
	{
		...
		if (cmd != "-" && cmd != "--" && cmd.StartsWith('-')) {
			throw new PicoArgsException(PicoArgsErrorCode.InvalidCommand, $"Expected command not \"{cmd}\"");
		}
	...
	{
		if (argList.Count > 0) {
			throw new PicoArgsException(PicoArgsErrorCode.UnrecognisedParameters, $"Unrecognised parameter(s): {string.Join(", ", argList)}");
		}
	...
	{
		if (finished) {
			throw new PicoArgsException(PicoArgsErrorCode.AlreadyFinished, "Cannot use PicoArgs after calling Finished()");
		}
	}
	...
	private static void ValidateInputParam(ReadOnlySpan<char> arg)
	{
		if (arg == "-") {
			...
			throw new PicoArgsException(PicoArgsErrorCode.InvalidParameter, "Parameter should not be a single dash");
		}
		if (arg.StartsWith("---")) {
			// eg ---something is not valid
			throw new PicoArgsException(PicoArgsErrorCode.InvalidParameter, $"Parameter should not start with 3 dashes: {arg}");
		}
		if (arg.Length == 3 && arg.StartsWith("--")) {
			// eg --a is not valid
			throw new PicoArgsException(PicoArgsErrorCode.InvalidParameter, $"Long options must be 2 characters or more: {arg}");
		}
	}
}
...
{
	public PicoArgsErrorCode Code { get; init; }

	public PicoArgsException(PicoArgsErrorCode code, string message) : base(message) => this.Code = code;
